...
export default Route.extend({
  ...
  model(params) {
    ...
    if (pageScope === 'cluster') {
      ...
      return this.loadGlobalResource(groupId);
    }
  },
  isMonitoringEnabled() {
    ...
    if (ps === 'cluster') {
      ...
      return this.settings.globalMonitoringEnabled === 'true'
    }
  },
  ...
  loadGlobalResource(groupId) {
    const globalStore = this.globalStore;

    return hash({
      notifiers:  globalStore.findAll('notifier'),
      alertRules: globalStore.find('globalAlertRule'),
      alertGroup: globalStore.find('globalAlertGroup', groupId),
    }).then(({
      notifiers, alertRules, alertGroup
    }) => {
      return {
        notifiers,
        alertRules: alertRules.filter((g) => g.groupId === groupId).map((a) => a.clone()),
        alertGroup: alertGroup.clone(),
        mode:       'edit',
        level:      'group',
      }
