...
export default Route.extend(EditOrClone, {
  ...
  model(params, transition) {
    ...
    if ( pageScope === 'cluster' ) {
      ...
      return this.loadGlobalResource(id, groupId);
    }
  },
  ...
  getNewGlobalAlert(clusterId, groupId) {
  ...
  loadGlobalResource(id, groupId) {
    const globalStore = this.globalStore;
    let newAlert;

    if ( id ) {
      newAlert = globalStore.find('globalAlertRule', id)
        .then(( alert ) => alert.cloneForNew());
    } else {
      newAlert = this.getNewGlobalAlert(groupId);
    }

    const opt = { filter: { clusterId: null } };
