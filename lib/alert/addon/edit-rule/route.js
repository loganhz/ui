...
export default Route.extend(EditOrClone, {
  ...
  model(params) {
    ...
    if (pageScope === 'cluster') {
      ...
      return this.loadGlobalResource(ruleId, groupId);
    }
  },
  isMonitoringEnabled() {
  ...
  loadGlobalResource(ruleId, groupId) {
    const globalStore = this.globalStore;

    return hash({
      notifiers:  globalStore.findAll('notifier'),
      alertRule:  globalStore.find('globalAlertRule', ruleId),
      alertGroup: globalStore.find('globalAlertGroup', groupId),
    }).then(({
      notifiers, alertRule, alertGroup
    }) => {
      return {
        notifiers,
        alertGroup,
        alertRule: alertRule.clone(),
        mode:      'edit',
        level:     'rule',
      }
    });
